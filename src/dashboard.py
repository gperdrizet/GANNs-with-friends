...
def get_sample_images(samples_dir: Path, limit: int = None):
    """Get sample images, sorted by iteration (newest first).
    ...
    Returns:
        List of (iteration_number, path) tuples, sorted by iteration descending
    """
    if not samples_dir.exists():
        return []
    
    images = []
    for img_path in samples_dir.glob('iteration_*.png'):
        try:
            iteration = int(img_path.stem.split('_')[-1])
            images.append((iteration, img_path))
        except ValueError:
            continue
...
def main():
    ...
    # Get all workers (used by multiple sections)
    ...
    if all_workers:
        ...
        st.info("Waiting for workers to connect.")
    
    # ==================== Throughput ====================
    st.markdown("---")
    st.subheader("Throughput")
    
    # Get loss history for throughput calculation
    loss_history = db.get_loss_history()
    
    if all_workers and len(loss_history) >= 2:
        # Calculate aggregate totals
        total_images = sum(w.get('total_images') or 0 for w in all_workers)
